...
A more complex example would be to compute a 2D FFT for each 64 x 64 subtile of the grid that has an input buffer with a raster grid of 1024 x 1024 monochrome pixel values. Specifying strides allows you to treat each horizontal band of 1024 x 64 pixels as an array of 16 64 x 64 matrixes, and process an entire band with a single call @ref clfftEnqueueTransform(). (Specifying strides is not quite flexible enough to transform the entire grid of this example with a single kernel execution.) It is possible to create a IPlan to compute arrays of 64 x 64 2D FFTs, then specify three strides: [1, 1024, 64]. The first stride, 1, indicates that the rows of each matrix are stored consecutively; the second stride, 1024, gives the distance between rows, and the third stride, 64, defines the distance between two matrices. Then call @ref clfftEnqueueTransform() 16 times – once for each horizontal band of pixels.
...
<ul>
	...
	<ol>
   		 <li> Create an FFT IPlan object.
           			To create an FFT IPlan object, do either of the following.
		<ul>
			...
			object.
    <li> Complete the specification of all the IPlan parameters by calling various parameter-setting
...
<ol>
	<li> Create the clFFT IPlan and initialize the standard clFFT parameters.
